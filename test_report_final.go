...
func generateFinalReport() {
	...
	// 保存到文件
	reportContent := generateReportFile()
	fmt.Println("\n💾 测试报告已保存到 mysqlctl_test_report.txt")
	fmt.Println(strings.Repeat("=", 80))
	
	// 写入文件
	WriteFile("mysqlctl_test_report.txt", reportContent)
}

func generateReportFile() string {
	content := "MySQLCTL 测试报告\n"
	...
	content += fmt.Sprintf("总测试用例: %d\n", total)
	content += fmt.Sprintf("通过: %d\n", passed)
	content += fmt.Sprintf("失败: %d\n", total-passed)
	content += fmt.Sprintf("通过率: %.2f%%\n\n", float64(passed)*100/float64(total))
	...
	for _, r := range results {
		...
		content += fmt.Sprintf("  %s - %s", status, r.Name)
		if r.Message != "" {
			content += fmt.Sprintf(" (%s)", r.Message)
		}
		content += "\n"
	}
